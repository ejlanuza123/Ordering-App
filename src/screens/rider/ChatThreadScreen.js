// src/screens/rider/ChatThreadScreen.js
...
const RiderChatThreadScreen = ({ route, navigation }) => {
  ...
  const handleFinishConversation = async () => {
    ...
    try {
      ...
      setConversation((prev) => ({ ...prev, is_closed: true }));
    } catch (err) {
  ...
  const getHeaderTitle = () => {
    ...
    if (conversation?.type === 'admin_rider') return 'Admin Support';
    return otherParticipant?.profiles?.full_name || 'Customer';
  };
...
export default RiderChatThreadScreen;